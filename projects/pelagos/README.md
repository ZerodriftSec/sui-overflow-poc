...
fully funded structured notes — all priced off **real on-chain liquidity** and minted, **wallet-signed**,
on **Sui testnet**.
...
| **Range Strips** | Three execution surfaces over the live DeepBook book: bought settlement strips; PLP counterparty-liquidity strategies with optional downside, two-way, or center hedges; and fully funded structured notes with terminal participation, range coupons, expiry knock-in/out terms, two-way buffers, and a discrete three-observation autocall whose coupon budget comes from a live final-expiry DeepBook premium. No fixed APY or guaranteed principal is presented. |
...
> builder), while Range Strips adds prebuilt recipes, PLP counterparty liquidity, and funded notes.
...
- **DeepBook Predict *is* the settlement venue, and it's Sui-native.** Every option, vol structure,
  range strip, and dUSDC structured-note strip leg is a real DeepBook Predict **range** position — priced
...
- **Programmable Transaction Blocks make multi-leg products one signature.** A dUSDC funded note
  atomically splits capital into a PLP reserve **and** a live range strip with an explicit execution
  buffer — in a *single* wallet signature. The in-app faucet tops a wallet with
...
- **Woven into Sui DeFi, not just deployed on it.** The vol desk marks BTC against Sui-native venues
  first — **Bluefin** perp, **DeepBook** XBTC/USDC CLOB, **Pyth** on Sui — while the yield surface owns
  transferable Predict PLP shares and optional live range hedges. Carry is the realized PLP NAV change,
  never a fabricated fixed APY.
...
- **`pelagos_strategies`** — a `structured_note` primitive (reserve-floor targeting + admin settlement).
...
**Collateral:** most products can use either of two independent testnet rails selected per order;
Autocall 3 is mUSDC-only. Both units are displayed in USD denomination, but there is no swap or peg.
**dUSDC** (DeepBook Predict's quote asset, faucet-gated) routes through Predict directly; **Pelagos
USDC** (`MOCK_USDC`, freely mintable) settles on Pelagos's own generic `Vault<T>` using a canonical
payoff schedule derived from the live DeepBook reference book. It is an isolated testnet rail, not
Predict PLP ownership, so demos never bottleneck on the dUSDC faucet.
