...
- **On-chain (Sui testnet)** — every package + object ID is committed in `backend/.env`
  or the managed Predict manifest and documented in **[`DEPLOYMENT.md`](DEPLOYMENT.md)**:
  `pelagos_sui` `0x598434be…`, `pelagos_vault` `0xcaff49f8…`, `pelagos_strategies`
  `0x30932e4e…`, deployer/admin wallet `0xcad0f800…`, and the 2026-07-18 managed
  DeepBook Predict package `0xf5716c88…`). Predict is an exact deployment of Mysten's official
  `predict-testnet-4-16` source; Pelagos operates its authorized oracle feed.
- **Data sources** — direct Sui gRPC + `devInspect`, Deribit, Polymarket (Gamma + CLOB),
  DeFiLlama, and Coinbase — all live, no market-data keys needed.
