...
export async function getBundleById(id: string): Promise<Bundle | null> {
  // On-chain vault labels may carry the human PBU name instead of the database
  // UUID. Resolve those as names instead of sending invalid UUID syntax to
  // Postgres (and producing a noisy error on every portfolio poll).
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(id)) {
    return getBundleByName(id);
  }
  const { data, error } = await supabase
    .from('bundles')
    .select('*')
