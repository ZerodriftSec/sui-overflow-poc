...
async function main() {
  if (process.env.LIVE_SMOKE !== '1') {
    throw new Error('Refusing live writes. Re-run with LIVE_SMOKE=1 after confirming the testnet transactions.');
  }
  ...
  const depTx = Transaction.from(dep.tx_bytes);
  ...
  const redTx = Transaction.from(red.tx_bytes);
