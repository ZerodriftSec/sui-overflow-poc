...
async function main() {
  ...
  if (!oracle) {
    console.error(`\n✗ No active ${underlying} oracle from the read facade; cannot price a trade.`);
    process.exit(1);
  }
  ...
  // 1. Reuse this owner's manager if the active read source knows one; else create fresh.
  ...
  // 3. Read the resulting position (best-effort; a public read source may lag).
  step(3, 'read manager positions (best-effort)');
  try {
