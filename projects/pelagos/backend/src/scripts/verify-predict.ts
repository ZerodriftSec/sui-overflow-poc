 ...
 *
 * Checks, in order:
 *   1. the active Predict read facade is reachable
...
async function main() {
  ...
  // 1. Active read source reachable
  try {
    const cfg = await predict.predictServer.config();
    ok('Predict read facade reachable', { keys: Object.keys(cfg).slice(0, 6) });
  } catch (err) {
    bad('Predict read facade reachable', err);
  }

  // 2. Active oracle
  const oracle = await predict.findActiveOracle(underlying).catch(() => null);
  if (!oracle) {
    bad(`active ${underlying} oracle found`, 'none returned by the active read source');
  } else {
