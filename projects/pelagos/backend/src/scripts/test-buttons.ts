...
async function signExec(txJson: string): Promise<string> {
...
async function main() {
  if (process.env.BUTTON_TEST_LIVE !== '1') {
    throw new Error('Refusing live writes. Re-run with BUTTON_TEST_LIVE=1 after confirming the testnet transactions.');
  }
  ...
  try {
    ...
    pass('PPN Deploy protected note', `digest ${digest}, vault ${ppnVaultId ?? '(read-pending)'}`);
  } catch (e) {
  ...
  // 6) PPN maturity guard — the receipt opened above is locked for 30 days.
  log('\n[6] PPN early exit guard  /api/ppn/onchain/redeem/prepare -> NOT_MATURED');
  try {
    await api<{ tx_bytes?: string; vault_id?: string | null }>(
      'POST',
    ...
    fail('PPN early exit guard', 'unexpectedly prepared a redemption before maturity');
  } catch (e) {
    const message = (e as Error).message;
    if (/not[_ -]?matured|maturity/i.test(message)) pass('PPN early exit guard', 'NOT_MATURED enforced');
    else fail('PPN early exit guard', message);
  }
