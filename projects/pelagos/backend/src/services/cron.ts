...
import { getActiveMarketSnapshot, getPolymarketBasketNAVs } from './polymarket';
...
const RESOLUTION_SWEEP_INTERVAL_MS = 60 * 60 * 1000;
let lastResolutionSweepAt = Date.now();
...
async function refreshAllBundles(): Promise<void> {
  ...
  try {
    const [bundles, polyNAVs, activeMarketSnapshot] = await Promise.all([
      getAllBundles(),
      getPolymarketBasketNAVs(),
      getActiveMarketSnapshot(),
    ]);
    ...
    const refreshBundle = async (
      bundle: typeof activeBundles[number],
        ...
        try {
          ...
          const navResult = await getLiveNAV(bundle.id, activeMarketSnapshot);
          if (navResult) {
    ...
    // Resolution is low-frequency state transition work. Running it per bundle
    // every two minutes tripled Gamma traffic and caused sustained 429s. Sweep
    // serially once an hour; regular NAV refreshes use the coalesced snapshot.
    if (Date.now() - lastResolutionSweepAt >= RESOLUTION_SWEEP_INTERVAL_MS) {
      lastResolutionSweepAt = Date.now();
