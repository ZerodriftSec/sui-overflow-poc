...
// 3 SUI. The heavy dUSDC DeepBook Predict opens pin a gas budget that SCALES with
// leg count (openGasBudget in structured.ts): up to openGasBudget(12) = 2.29 SUI for
// the widest basket. That budget is RESERVED (not spent) from the wallet's SUI at sign
// time, so the grant MUST stay >= the max pin + a gas coin + slack, or the reservation
// itself fails (the earlier 0.4 SUI couldn't even reserve a 6-leg open, so opens
// reverted on-chain with InsufficientGas). Net charge after the storage rebate is a
// small fraction of the reservation (a 6-leg open nets ~0.2 SUI). INVARIANT: keep
// SUI_GRANT_MIST >= openGasBudget(12) + a gas coin. The cheap mUSDC vault rail uses a
// fraction of this. Override with SUI_GRANT_MIST.
const SUI_GRANT_MIST = BigInt(process.env.SUI_GRANT_MIST ?? 3_000_000_000); // 3 SUI
