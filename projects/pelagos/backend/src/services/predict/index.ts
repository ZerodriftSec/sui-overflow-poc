...
export * from './server';
export { predictFeedStatus, pushPredictOracleTick } from './feed';
...
async function execute(tx: Transaction): Promise<ExecResult> {
  ...
  // gRPC execution finality can precede read visibility for a newly-created
  // shared object by a short interval. Every composed write flow may reference
  // its previous output immediately, so make that sequencing guarantee here.
  ...
  return {
    digest: res.digest,
    status,
    object_changes: confirmed.objectChanges ?? res.objectChanges ?? [],
    events: (confirmed.events ?? res.events ?? []).map((e) => ({
      type: e.type,
...
export async function createManager(): Promise<ExecResult & { manager_id: string | null }> {
  ...
  const managerId = createdObjectId(result.object_changes, '::predict_manager::PredictManager');
  const owner = signerAddress();
  if (managerId && owner) {
    registerManagedManager({ managerId, owner, digest: result.digest });
  }
  return {
    ...result,
