...
// Gamma currently rejects offsets above 2,000 with 422. Results are sorted by
// volume, so the reachable 2,100 rows are the useful market universe anyway.
const GAMMA_MAX_OFFSET = 2_000;
 ...
 * per-page cap (100), we paginate via `offset` until we have enough.
 *
...
async function fetchMarketsRaw(params: {
  ...
  while (collected.length < want && offset <= GAMMA_MAX_OFFSET) {
...
const conditionMarketCache = new Map<string, { at: number; market: PolymarketMarket }>();
const conditionMarketInFlight = new Map<string, Promise<PolymarketMarket | null>>();
const CONDITION_MARKET_TTL_MS = 60_000;
...
export async function fetchMarketByConditionId(
  ...
  const cacheKey = conditionId.toLowerCase();
  const cached = conditionMarketCache.get(cacheKey);
  if (cached && Date.now() - cached.at < CONDITION_MARKET_TTL_MS) return cached.market;
  const existing = conditionMarketInFlight.get(cacheKey);
  if (existing) return existing;
  ...
  const pending = (async () => {
    ...
    const parsed = toPolymarketMarket(market);
    conditionMarketCache.set(cacheKey, { at: Date.now(), market: parsed });
    return parsed;
  })().finally(() => conditionMarketInFlight.delete(cacheKey));
  conditionMarketInFlight.set(cacheKey, pending);
  return pending;
}
...
export function marketProbability(market: PolymarketMarket): number | null {
...
export async function getMarketProbability(conditionId: string): Promise<number | null> {
  ...
  return market ? marketProbability(market) : null;
}

/** One coalesced active-market snapshot for cron valuation without per-leg fan-out. */
export async function getActiveMarketSnapshot(): Promise<Map<string, PolymarketMarket>> {
  const markets = await fetchMarkets({ limit: 5000, active: true, closed: false });
  return new Map(
    markets
      .filter((market) => market.condition_id)
      .map((market) => [market.condition_id.toLowerCase(), market]),
  );
}
