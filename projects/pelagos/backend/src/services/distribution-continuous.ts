...
const MOCK_USDC_TYPE =
  ...
  // Fallback pinned to the CURRENT mock_usdc package (matches vault/config.ts and
  // mock-usdc.ts). Previously pointed at the retired 0xa630b97e coin — a landmine
  // if MOCK_USDC_TYPE is ever left unset on a deploy.
...
const POOL_STORE_FILE = statePath('.distribution-pools.json');
...
function quoteCore(p: {
  ...
  // Expected value of the trade payoff under the MARKET distribution f — the same
  // measure the realized outcome is drawn from at settle (drawNormalSecure with
  // market_mu/market_sigma) and that the close mark uses. Pricing EV under the
  // trader's OWN target g instead would always look favourable (circular) and has
  // the opposite sign to the actual settlement expectation.
  const fMass = marketPdf.reduce((s, v) => s + v * dx, 0) || 1;
  const ev = tradeCurve.reduce((s, v, i) => s + v * (marketPdf[i] / fMass) * dx, 0);
...
// Minimum hold before a continuous position may be settled. Defence-in-depth on
// top of the secret-seeded draw: stops open→settle cycling to probe outcomes.
const CONTINUOUS_MIN_HOLD_MS = 8_000;

/**
 * Realized Normal draw seeded by a SERVER-ONLY secret via a cryptographic PRF
 * (HMAC-SHA256 → uniforms → Box-Muller). The secret is generated at open with
 * crypto.randomBytes and is NEVER returned to the client before settlement, so a
 * trader cannot predict or grind the outcome (the previous version seeded off the
 * trader-controlled open digest, which was fully predictable/grindable).
 */
function drawNormalSecure(mu: number, sigma: number, secret: string): number {
  const uniform = (ctr: number): number => {
    const h = createHmac('sha256', secret).update(`pelagos-realized:${ctr}`).digest();
    return h.readUIntBE(0, 6) / 2 ** 48; // 48 bits → [0,1)
  };
  const u1 = Math.max(uniform(1), 1e-9);
  const u2 = uniform(2);
...
export interface ContinuousPosition {
  ...
  open_digest: string;
  opened_at: number;
  realized_x: number; // 0 until settle; drawn at settle from settle_secret
  /** Server-only secret seeding the realized draw — NEVER serialized to clients pre-settle. */
...
const STORE_FILE = statePath('.distribution-positions.json');
...
export async function prepareContinuousOpen(args: {
  ...
  try {
    ...
    const bytes = await probe.build({ client: client.core });
...
function addressOwnerOf(o: unknown): string | null {
  return o && typeof o === 'object' && 'AddressOwner' in (o as object)
    ? (o as { AddressOwner: string }).AddressOwner
    : null;
}

/**
 * Verify a digest is a REAL escrow: in a successful tx, `owner` sent at least
 * `minRaw` MOCK_USDC to the protocol treasury. Without this, any successful tx
 * digest could forge an open and later mint a payoff on settle for free (the
 * collateral was never paid). Returns false on any mismatch.
 */
async function verifyEscrowDeposit(digest: string, owner: string, minRaw: number): Promise<boolean> {
  try {
    await getSuiClient().waitForTransaction({ digest, timeout: 12_000 }).catch(() => {});
    ...
    const treasury = treasuryAddress();
    const bc = tx.balanceChanges ?? [];
    const ownerOut = bc.find(
      (c) => c.coinType === MOCK_USDC_TYPE && addressOwnerOf(c.owner)?.toLowerCase() === owner.toLowerCase(),
    );
    const treasuryIn = bc.find(
      (c) => c.coinType === MOCK_USDC_TYPE && addressOwnerOf(c.owner)?.toLowerCase() === treasury.toLowerCase(),
    );
    const spent = ownerOut ? -Number(BigInt(ownerOut.amount)) : 0; // owner delta is negative
    const received = treasuryIn ? Number(BigInt(treasuryIn.amount)) : 0;
    const need = minRaw * 0.999; // allow rounding dust
    return spent >= need && received >= need;
  } catch {
    return false;
  }
}
...
export async function confirmContinuousOpen(args: {
  ...
  // Idempotent replay; reject reusing a digest already bound to another owner.
  const existing = store.get(args.digest);
  if (existing) {
    // Sanitize on replay too: returning the raw stored position would leak the
    // server-only settle_secret, letting a digest-replay pre-compute the realized
    // draw and revive the settle-vs-close grind. Strip it like the fresh path.
    if (existing.owner.toLowerCase() === args.owner.toLowerCase()) return sanitizeContinuous(existing);
    throw new Error('digest already used for another position');
  }
  const quote = quoteContinuous(args);
  // SECURITY: verify the escrow actually happened — this owner sent >= the required
  // collateral in mUSDC to the treasury in this tx. A success-only check let any
  // digest forge an open and free-mint a payoff on settle without paying collateral.
  const collateralRaw = Math.round(quote.collateral_required_usdc * 1e6);
  if (!(await verifyEscrowDeposit(args.digest, args.owner, collateralRaw))) {
    throw new Error('Open transaction did not escrow the required collateral to the treasury.');
  }
  // SECURITY: do NOT draw the realized outcome at open from anything the trader can
  // see or grind. Generate a server-only secret now; the draw happens at SETTLE
  // (see settleContinuousPosition), so the outcome is unknown until the trader
  // irrevocably settles — closing the pre-compute/grind exploit.
  const settleSecret = randomBytes(32).toString('hex');
  ...
  const pos: ContinuousPosition = {
    ...
    opened_at: Date.now(),
    realized_x: 0, // drawn at settle from settle_secret
    settle_secret: settleSecret,
    settled: false,
  };
  store.set(pos.id, pos);
  saveStore();
  return sanitizeContinuous(pos);
}

/** Strip the realized-draw secret from any client-facing position. */
function sanitizeContinuous(p: ContinuousPosition): ContinuousPosition {
  const { settle_secret: _omit, ...rest } = p;
  void _omit;
  return rest;
}
...
export async function settleContinuousPosition(args: { owner: string; positionId: string }): Promise<SettleResult> {
  ...
  // Anti-grind hold (defence-in-depth on top of the secret-seeded draw): a position
  // cannot be opened and instantly settled.
  if (Date.now() < pos.opened_at + CONTINUOUS_MIN_HOLD_MS) {
    throw new Error('Position is held briefly after open; settlement is available momentarily.');
  }
  // Draw the realized outcome NOW from the server-only secret — unknown to the trader
  // until they irrevocably settle. Legacy positions (opened before this change) have
  // no secret and keep their previously stored realized_x.
  if (pos.settle_secret) {
    pos.realized_x = Math.round(drawNormalSecure(pos.market_mu, pos.market_sigma, pos.settle_secret) * 100) / 100;
  }
