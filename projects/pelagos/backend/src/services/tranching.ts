...
export function quoteTranches(opts: QuoteTranchesInputs): TrancheQuote[] {
  ...
  // Ceiling-aware attachment spacing (mirrors frontend `attachmentPoints` in
  // app/app/tranche/_quote.ts). The naive `k2 = clamp01(k1 + GAP)` collapsed
  // the mezz to zero width once nav≥0.99 (clamp01 pins both k1 and k2 at the
  // 0.99 ceiling). When there's no room to push k2 up, pull k1 down instead so
  // senior/mezz/junior always partition [0,1] with strictly positive widths.
  const MIN_TRANCHE_GAP = 0.01;
  let k1 = clamp01(nav + ATTACH_K1_SIGMA * sigma);
  let k2 = clamp01(nav + ATTACH_K2_SIGMA * sigma);
  if (k2 - k1 < MIN_TRANCHE_GAP) {
    if (k1 + MIN_TRANCHE_GAP <= 0.99) {
      k2 = k1 + MIN_TRANCHE_GAP; // room to push k2 up (mid/low NAV)
    } else {
      k2 = 0.99; // at the ceiling: pull k1 down so junior [0.99,1] stays non-empty
