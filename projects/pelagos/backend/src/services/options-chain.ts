...
async function buildOptionsChain(underlying = 'BTC'): Promise<OptionsChain> {
  ...
  if (active.length === 0) throw new Error('no active oracles');
  ...
  const built = await Promise.all(
    active.map(async (o): Promise<OptionExpiry | null> => {
      ...
      const strikes: OptionStrikeRow[] = defs.map((d, i) => {
        ...
        const call = toQuote(callP, String(d.kRaw), String(farRaw), d.iv, gk.callDelta, gk);
        // Binary PARITY: a call(K) and put(K) on the SAME strike are exact
        // complements — at expiry exactly one of them pays $1 — so put(K) = 1 − call(K).
        // We DERIVE the put from the call rather than pricing its [floor,K] range
        // independently: that range's floor is clamped at the oracle min_strike on
        // long tenors, truncating its coverage so the two INDEPENDENT legs summed
        // BELOW $1 — a phantom static put-call arbitrage (buy both < $1, collect $1).
        // Deriving enforces no-arb (call.ask + put.ask = 1 + spread > 1) with the
        // bid/ask flipped (put.ask = 1 − call.bid) and matches the documented intent.
        const put: OptionQuote = call.quoted
          ? {
              mid: Number((1 - call.mid).toFixed(4)),
              bid: Number((1 - call.ask).toFixed(4)),
              ask: Number((1 - call.bid).toFixed(4)),
              iv: call.iv,
              delta: Number(gk.putDelta.toFixed(4)),
              gamma: Number((-gk.gamma).toFixed(6)),
              vega: Number((-gk.vega).toFixed(4)),
              theta: Number((-gk.theta).toFixed(4)),
              tradeable: call.tradeable,
              quoted: true,
              lower_strike: String(floorRaw),
              higher_strike: String(d.kRaw),
            }
          : toQuote(putP, String(floorRaw), String(d.kRaw), d.iv, gk.putDelta, { gamma: -gk.gamma, vega: -gk.vega, theta: -gk.theta });
        return { strike: d.strike, moneyness: d.moneyness, call, put };
      });
  ...
  if (expiries.length === 0) {
    throw new Error('active Predict oracles are not currently quoteable');
  }
