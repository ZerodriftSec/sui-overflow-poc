...
export function decodeVaultLabel(raw: unknown): string {
  if (Array.isArray(raw)) {
    return new TextDecoder().decode(Uint8Array.from(raw as number[]));
  }
  if (typeof raw !== 'string') return '';

  // Core gRPC renders vector<u8> JSON fields as base64, while JSON-RPC rendered
  // them as byte arrays. Only accept a base64 interpretation when it round-trips
  // and decodes to printable label text; ordinary labels remain untouched.
  try {
    const bytes = fromBase64(raw);
    const decoded = new TextDecoder('utf-8', { fatal: true }).decode(bytes);
    const normalized = (value: string) => value.replace(/=+$/, '');
    if (
      normalized(toBase64(bytes)) === normalized(raw) &&
      /^[\x20-\x7e]*$/.test(decoded)
    ) {
      return decoded;
    }
  } catch {
    // Not base64-encoded text; preserve the original JSON-RPC-era string.
  }
  return raw;
}
  ...
  for (const rail of rails) {
    ...
    do {
      ...
      for (const o of page.data) {
        ...
        const label = decodeVaultLabel(f.label);
        out.push({
...
export async function prepareRedeem(args: {
  ...
  label?: string;
  currency?: VaultCurrency;
  /** The PPN redeem route (/api/ppn/onchain/redeem) sets this: it BUILDS the tx
   *  here and enforces the maturity gate itself afterward, so it is allowed to
   *  target ppn: shares. Every other caller (the generic basket /api/deposit/redeem
   *  rail) leaves it false, so a maturity-locked note can never exit ungated. */
...
> {
  ...
  // Sim positions (label `sim:<product>:<id>`) share the same Vault<MOCK_USDC> but
  // settle by MINTING the band payoff (see services/sim-settlement.ts) — they must
  // NEVER be redeemed. A redeem returns only premium − fee (vault NAV is flat at 1.0)
  // and silently discards the structured strip/vol/option/dist payoff. So exclude
  // `sim:` shares from every redeem path, and reject an explicit sim share_id.
  // Protected notes / tranches (label `ppn:<kind>:<bundle>`) are MATURITY-LOCKED.
  // They must ONLY exit via /api/ppn/onchain/redeem, which enforces the maturity
  // gate (+ early-exit NAV/penalty rules). Redeeming one through this generic rail
  // would bypass the principal-protection lock entirely — an early redeem at flat
  // vault NAV. So exclude `ppn:` shares here, and reject an explicit ppn: target.
  if (args.share_id) {
    const byId = shares.find((s) => s.share_id === args.share_id);
    if (byId && byId.label.startsWith('sim:')) {
      throw new Error('Sim positions settle via /api/sim/settle (mint), not redeem.');
    }
    if (!args.allowProtected && byId && byId.label.startsWith('ppn:')) {
      throw new Error('Protected notes/tranches redeem via /api/ppn/onchain/redeem (maturity-gated), not this rail.');
    }
  }
  if (!args.allowProtected && args.label && args.label.startsWith('ppn:')) {
    throw new Error('Protected notes/tranches redeem via /api/ppn/onchain/redeem (maturity-gated), not this rail.');
  }
  const redeemable = shares.filter((s) => !s.label.startsWith('sim:') && (args.allowProtected || !s.label.startsWith('ppn:')));

  let target = args.share_id
    ? redeemable.find((s) => s.share_id === args.share_id)
    : args.label
      ? redeemable.find((s) => s.label === args.label)
      : undefined;
  if (!target) {
    if (redeemable.length === 0) throw new Error(`No redeemable vault positions for ${args.owner}`);
    // default: the largest REDEEMABLE position (never a sim: share)
    target = [...redeemable].sort((a, b) => b.shares - a.shares)[0];
  }
