 ...
 */
import { Leg, NAVResult, PolymarketMarket } from '../types';
import { calculateNAV, calculateIssuePrice } from './nav';
import { getMarketProbability, fetchMarketByConditionId, marketProbability } from './polymarket';
import {
  getLegsByBundleId,
...
export async function getLiveNAV(
  bundleId: string,
  marketSnapshot?: ReadonlyMap<string, PolymarketMarket>,
  ...
  const updates = await Promise.allSettled(
    activeLegs.map(async (leg) => {
      const snapshotMarket = marketSnapshot?.get(leg.market_id.toLowerCase());
      const prob = snapshotMarket
        ? marketProbability(snapshotMarket)
        : await getMarketProbability(leg.market_id);
      if (prob !== null) {
