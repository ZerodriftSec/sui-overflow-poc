...
export async function mintMockUsdc(
  ...
  // Refuse to silently UNDER-mint: if the requested amount exceeds the 16-chunk
  // (16M mUSDC) ceiling, throw so the caller (e.g. settleSim) surfaces an error
  // instead of booking/returning a payoff larger than what was actually minted.
  if (raw > 0n) {
    throw new Error(`mint amount exceeds the per-call ceiling (16,000,000 mUSDC); refusing to under-mint ${displayAmount}.`);
  }
