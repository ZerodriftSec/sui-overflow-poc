...
// wallet_address is a canonical 0x-prefixed Sui address: exactly 64 hex chars
// after 0x (full form is 66 chars). A looser 1–64-hex match let short-but-hex
// inputs (e.g. 0xabc) through, which the Sui RPC then rejected with "Invalid
// params" deep in the call → a 500; require the full-length form so malformed
// input is rejected up front with a clean 400.
const suiAddress = z
  .string()
