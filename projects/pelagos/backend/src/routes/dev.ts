...
// Global aggregate limiter: the per-IP cap alone can't stop a botnet/proxy farm
// (each fresh IP gets its own 10/min budget), so this second limiter bounds TOTAL
// faucet throughput across ALL IPs to cap the worst-case float drain rate. Keyed
// on a constant so every caller shares one counter.
const faucetGlobalLimiter = rateLimit({
  windowMs: 60 * 60 * 1000,
  limit: Number(process.env.FAUCET_GLOBAL_HOURLY_LIMIT ?? 12),
  standardHeaders: 'draft-7',
  legacyHeaders: false,
  keyGenerator: () => 'global',
  message: { error: 'The shared demo faucet has reached its hourly limit; please try again later' },
});
...
// Hard ceiling on how much mUSDC a single wallet may hold via the faucet. Gates
// on the LIVE on-chain balance (so it survives restarts and accounts for spent
// funds): a wallet can refill what it trades away, but can never accumulate an
// unbounded mUSDC stack. NOTE: mock_usdc::mint is permissionless on-chain, so this
// only bounds the backend faucet — a capped on-chain mint needs a Move change.
const MAX_MUSDC_BALANCE = 1_000_000;
 ...
 * operator float), and 3 SUI for gas so the user can sign their first tx.
 * Powers the header faucet's Mint button.
 */
router.post('/faucet', faucetGlobalLimiter, faucetIpLimiter, async (req: Request, res: Response) => {
  const { walletAddress } = req.body as { walletAddress?: string };
  if (!walletAddress) return res.status(400).json({ error: 'walletAddress is required' });
  // Canonicalize up front so the cap read targets the SAME address as the mint
  // (a short/odd form would otherwise make usdcBalance throw and skip the cap).
  if (!isValidSuiAddress(normalizeSuiAddress(walletAddress)))
    return res.status(400).json({ error: 'invalid walletAddress' });
  const wallet = normalizeSuiAddress(walletAddress);
  const wait = faucetCooldownRemaining(wallet);
  if (wait != null) return res.status(429).json({ error: `Faucet cooldown: try again in ${wait}s` });
  // Balance cap: refuse if the wallet already holds the ceiling of mUSDC. Fail
  // CLOSED — if the balance can't be read we can't prove we're under the cap, so
  // deny rather than mint unbounded (canonical address above keeps this from
  // tripping on normal calls; 503 then only fires on a true RPC outage).
  try {
    if ((await usdcBalance(wallet)) >= MAX_MUSDC_BALANCE) {
      return res.status(429).json({
        error: `mUSDC balance cap reached (${MAX_MUSDC_BALANCE.toLocaleString()}). Spend some before requesting more.`,
      });
    }
  } catch {
    return res.status(503).json({ error: 'Unable to verify mUSDC balance cap (RPC unavailable); please retry.' });
  }
  // Reserve the cooldown slot synchronously to close the concurrent-call window,
  // then roll it back if the dispense throws.
  recordDispense(wallet);
  try {
    const result = await dispenseTestFunds(wallet);
    res.json({ chain: 'sui', walletAddress: wallet, ...result });
  } catch (err) {
    clearDispense(wallet);
...
router.post('/airdrop-mock-usdc', faucetGlobalLimiter, faucetIpLimiter, async (req: Request, res: Response) => {
  ...
  if (!isValidSuiAddress(normalizeSuiAddress(walletAddress)))
    return res.status(400).json({ error: 'invalid walletAddress' });
  const wallet = normalizeSuiAddress(walletAddress);
  // If `amount` is supplied it must be a positive finite number; reject an
  // explicit 0/negative/non-numeric rather than silently coercing it to 1000
  // (omitting the field entirely still defaults to 1000 for the FE button).
  ...
  if (wait != null) return res.status(429).json({ error: `Faucet cooldown: try again in ${wait}s` });
  // Balance cap: refuse if the wallet already holds the ceiling of mUSDC. Fail
  // CLOSED — deny rather than mint unbounded when the balance can't be read.
  try {
    if ((await usdcBalance(wallet)) >= MAX_MUSDC_BALANCE) {
      return res.status(429).json({
        error: `mUSDC balance cap reached (${MAX_MUSDC_BALANCE.toLocaleString()}). Spend some before minting more.`,
      });
    }
  } catch {
    return res.status(503).json({ error: 'Unable to verify mUSDC balance cap (RPC unavailable); please retry.' });
  ...
  const amt = typeof amount === 'number' ? Math.min(amount, MAX_AIRDROP) : 1000;
  // Reserve the cooldown slot synchronously to close the concurrent-call window,
  // then roll it back if the mint throws.
  recordDispense(wallet);
  try {
    const result = await mintMockUsdc(wallet, amt);
    res.json({ chain: 'sui', walletAddress: wallet, ...result });
  } catch (err) {
    clearDispense(wallet);
 ...
 */
router.post('/airdrop-dusdc', faucetGlobalLimiter, faucetIpLimiter, async (req: Request, res: Response) => {
  const { walletAddress, amount } = req.body as { walletAddress?: string; amount?: number };
  if (!walletAddress) return res.status(400).json({ error: 'walletAddress is required' });
  // Canonicalize so the cooldown key + grant recipient are the same address form.
  if (!isValidSuiAddress(normalizeSuiAddress(walletAddress)))
    return res.status(400).json({ error: 'invalid walletAddress' });
  const wallet = normalizeSuiAddress(walletAddress);
  const wait = faucetCooldownRemaining(wallet);
  ...
  recordDispense(wallet);
  try {
    const result = await dispenseDusdc(wallet, amt);
    res.json({ chain: 'sui', walletAddress: wallet, ...result });
  } catch (err) {
    clearDispense(wallet);
...
router.get('/dusdc-balance/:walletAddress', async (req: Request, res: Response) => {
  ...
  // clean 400 instead of a raw RPC-driven 500 — parity with /balances below.
  // Canonical Sui address = 0x + exactly 64 hex chars.
  if (!/^0x[0-9a-fA-F]{64}$/.test(wallet)) {
    return res.status(400).json({ error: 'invalid address' });
  }
  try {
    res.json({ chain: 'sui', wallet, dusdc: await dusdcBalance(wallet) });
  } catch (err) {
    // Defense-in-depth: if the RPC still rejects the address (e.g. "Invalid Sui
    // address"), map it to a 400 rather than leaking a 500.
    const message = (err as Error).message;
    if (/invalid.*address|invalid params/i.test(message)) {
      return res.status(400).json({ error: 'invalid address' });
    }
    res.status(500).json({ error: `Failed to read dUSDC balance: ${message}` });
  }
});
...
router.get('/balances/:walletAddress', async (req: Request, res: Response) => {
  const wallet = req.params.walletAddress;
  // Reject malformed addresses before any Sui RPC so a bad param returns a
  // clean 400 instead of a raw RPC-driven 500. Canonical Sui address = 0x +
  // exactly 64 hex chars; short-but-hex inputs (e.g. 0x123) otherwise reach the
  // RPC and throw "Invalid params" → 500.
  if (!/^0x[0-9a-fA-F]{64}$/.test(wallet)) {
    return res.status(400).json({ error: 'invalid address' });
  }
  try {
