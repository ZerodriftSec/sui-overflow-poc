...
function sendError(res: Response, err: unknown, code = 500) {
  ...
  // Map an upstream predict-server "404 Not Found / Resource not found" (an
  // unknown/stale manager or oracle id, a not-yet-created PredictManager, etc.)
  // to a clean 404 instead of re-emitting it as a 500. A genuinely unknown id is
  // a not-found, not a server fault — and 5xx alerting shouldn't fire on it.
  const status =
    code === 500 && /\b404\b|Not Found|Resource not found/i.test(message) ? 404 : code;
...
router.get('/status', async (_req: Request, res: Response) => {
  try {
    ...
    res.json({
      ...
      feed_status: predict.predictFeedStatus(),
    });
  } catch (err) {
    sendError(res, err);
  }
...
router.get('/oracles/:id/state', async (req: Request, res: Response) => {
  // A malformed id (not 0x-hex) can never be a real oracle — reject it as a
  // not-found rather than forwarding garbage to the predict-server. A well-formed
  // but unknown/stale id falls through and sendError maps the upstream 404.
...
router.get('/vol-surface', async (req: Request, res: Response) => {
  try {
    ...
    // Validate the strikes param inline (early 400) rather than `throw`ing into
    // the shared catch, which would surface a client validation error as a 500.
    // Genuine internal failures from buildVolSurface still fall through to 500.
    if (!Number.isFinite(strikesPct) || strikesPct <= 0 || strikesPct >= 1) {
      return res.status(400).json({ error: 'strikes must be in (0, 1)' });
    }
...
router.get('/managers/:id/summary', async (req: Request, res: Response) => {
  // A malformed id (not 0x-hex) can never be a real manager — 404 it rather than
  // forwarding garbage. A well-formed but unknown id (e.g. a freshly-connected
  // wallet with no PredictManager yet) falls through; sendError maps the
  // upstream 404 to a clean 404 instead of a 500.
  if (!isObjectId(req.params.id)) {
...
/** A manager's idle (reclaimable) dUSDC balance, on-chain — feeds the Reclaim UI. */
router.get('/managers/:id/idle-dusdc', async (req: Request, res: Response) => {
  try {
    const idle = await structured.managerIdleBalance(req.params.id);
    res.json({ manager_id: req.params.id, idle_raw: idle.toString(), idle: Number(idle) / 1e6 });
  } catch (err) {
    sendError(res, err, 400);
  }
});
...
/**
 * Owner-signed reclaim of idle dUSDC collateral from a manager back to the wallet.
 * NOT admin: the connected wallet (the manager owner) signs — the Move fn aborts
 * unless tx sender == manager.owner. Optional amount_raw; defaults to full idle.
 */
router.post('/manager/withdraw/prepare', async (req: Request, res: Response) => {
  try {
    const body = req.body as Record<string, unknown>;
    if (!isObjectId(body.owner)) throw new Error('owner (0x...) is required');
    if (!isObjectId(body.manager_id)) throw new Error('manager_id (0x...) is required');
    const amountRaw = rawAmount(body, 'amount_raw', 'amount_ui') ?? undefined;
    res.json(await structured.prepareManagerWithdraw({ owner: body.owner as string, managerId: body.manager_id as string, amountRaw }));
  } catch (err) {
    sendError(res, err, 400);
  }
});
