...
// lend/withdraw/borrow/repay mutate the SHARED in-memory pool (total_deposits /
// total_borrows), which drives utilization and the borrow/supply APYs every
// caller sees in the GET snapshot. The FE lending surface is display-only (no
// /app/lending page wires these), so an anonymous caller hitting them only griefs
// the rates shown to everyone. Gate them behind the same admin secret as the
// other operator-mutation routes; the GET snapshot and read-only /quote stay public.
