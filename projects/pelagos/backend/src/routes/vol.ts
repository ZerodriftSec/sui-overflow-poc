...
import { computeVolGreeks, strategyProfile, customVolProfile, stripCollapsed, tightenSpanForCollapse, symmetrizeStrip, type VolSide, type VolStrategy } from '../services/predict/volatility';
...
router.post('/quote', async (req: Request, res: Response) => {
  try {
    ...
    // `position_size_usd` is an accepted alias for the strip size. Reject a
    // non-finite/≤0 value up front rather than silently ignoring it and falling
    // back to the $100 default (which produced a nonsensical "size accepted" 200).
    if (body.position_size_usd !== undefined) {
      const positionSize = Number(body.position_size_usd);
      if (!Number.isFinite(positionSize) || positionSize <= 0) {
        return res.status(400).json({ error: 'position_size_usd must be a positive number' });
      }
    }
    const notionalRaw = body.notional_usd ?? body.budget_usd ?? body.position_size_usd ?? 100;
    const notionalUsd = Number(notionalRaw);
    if (!Number.isFinite(notionalUsd) || notionalUsd <= 0) {
      return res.status(400).json({ error: 'notional_usd must be a positive number' });
    }
    // Keep enough raw dUSDC for the eight-leg sizing pass to produce real exposure.
    // Protected notes intentionally buy small, yield-funded sleeves (the default
    // $10 note budgets roughly $0.10-$0.20), so a whole-dollar floor rejects a
    // perfectly usable on-chain strip. One cent remains comfortably above dust.
    if (notionalUsd < 0.01) {
      return res.status(400).json({ error: 'notional_usd must be at least $0.01' });
    }
    ...
    const buildStrip = (spanSigma: number) =>
      structured.previewStrip({
    ...
    let span = profile.spanSigma;
    let strip = await buildStrip(span);
    // B3: a SYMMETRIC long-gamma strip can silently drop an OTM wing under SVI skew
    // when an outer bucket falls below the 2% mint floor — turning a delta-neutral
    // strangle into a one-sided directional strip. Tighten the span until the strip
    // is intact, then symmetrize any residual drop so it stays delta-neutral. A
    // deliberately one-sided custom sculpt (profile.symmetric === false) is left as-is.
    if (profile.symmetric) {
      for (let guard = 0; stripCollapsed(strip) && guard < 6; guard++) {
        const next = tightenSpanForCollapse(span);
        if (next === span) break; // hit the symmetric-span floor
        span = next;
        strip = await buildStrip(span);
      }
      if (stripCollapsed(strip)) strip = symmetrizeStrip(strip);
    }
...
router.get('/hedge', async (req: Request, res: Response) => {
  try {
    ...
    // Reject a non-finite delta_btc (e.g. the literal "Infinity"/"1e400") rather
    // than letting it flow into quoteHedge and serialize as null in priced fields.
    const deltaBtc = Number(req.query.delta_btc ?? 0);
    if (!Number.isFinite(deltaBtc)) {
      return res.status(400).json({ error: 'delta_btc must be a finite number' });
    }
