...
import {
  ConnectButton as DappConnectButton,
  ConnectModal,
  useCurrentAccount,
 ...
 */
export function ConnectButton({ variant = "header" }: { variant?: "header" | "block" }) {
  const account = useCurrentAccount();
  const [open, setOpen] = useState(false);

  return (
    <div className="pelagos-connect" data-variant={variant} style={{ display: "flex" }}>
      <style>{`
      ...
      {account ? (
        ...
        <ConnectModal
          ...
          trigger={<button type="button" data-dapp-kit>Connect Wallet</button>}
        />
      )}
    </div>
  );
