...
export function OrderBook({ oracleId, levels = 16 }: { oracleId?: string; levels?: number }) {
  ...
  const requestKey = `${oracleId ?? ""}:${levels}:${tf}`;
  const [bookState, setBookState] = useState<{
    key: string;
    quote: StripQuote | null;
    status: "ok" | "error";
  } | null>(null);
  ...
  useEffect(() => {
    ...
    const load = () =>
      // Keep the depth probe inside the live PLP risk envelope. The old $2,000
      // probe exceeded the managed testnet pool's capacity and made a healthy
      // book render as unavailable on every PPN page.
      stripPreview({ oracle_id: oracleId, n: levels, span_sigma: span, budget_usd: 10 })
        .then((q) => {
          if (alive) {
            setBookState({ key: requestKey, quote: q, status: "ok" });
            setPulse((p) => p + 1);
          }
        })
        .catch(() => {
          if (alive) setBookState({ key: requestKey, quote: null, status: "error" });
        });
    load();
    const timer = window.setInterval(load, 4500);
    return () => { alive = false; window.clearInterval(timer); };
  }, [oracleId, tf, levels, requestKey]);

  // Hide a prior timeframe's snapshot while its replacement is loading.
  const quote = bookState?.key === requestKey ? bookState.quote : null;
  const status = bookState?.key === requestKey ? bookState.status : "loading";
...
export function OpenButton({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      <button className="mk-open" disabled={busy || disabled || short} onClick={onOpen}>
...
export function PpnPanel({ wallet }: { wallet: Wallet }) {
  ...
  async function open() {
    ...
    setResult(null);
    try {
      // mUSDC deposits the full budget and settles to the selected floor plus
      // realized upside. The catch-all band pays that floor when BTC misses.
      if (currency === "mUSDC") {
      ...
      <aside style={{ display: "flex", flexDirection: "column", gap: 16, minWidth: 0 }}>
        ...
        <div style={PANEL}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(0,1fr)", gap: 14, marginTop: 14 }}>
            <Stat label="Selected floor" value={quote ? usd(quote.protected_principal_raw) : "—"} color={C.green} />
          ...
          <div style={{ display: "grid", gap: 10, marginTop: 16, paddingTop: 14, borderTop: `0.5px solid ${C.border}` }}>
            <Row k="Floor share" v={quote ? pct(quote.protection_pct) : "—"} />
      ...
      <main style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        <div style={{ ...PANEL, flex: 1, display: "flex", flexDirection: "column" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 14 }}>
            <Cap>Upside strip ladder · on-chain MM</Cap>
        ...
        <div style={PANEL}>
          <OpenButton
            ...
            busy={busy}
            disabled={!quote}
            label={err ? "DeepBook pricing unavailable" : `Open protected note · ${quote ? usd(quote.budget_raw) : ""}`}
