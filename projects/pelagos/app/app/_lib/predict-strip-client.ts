...
/** Idle (reclaimable) dUSDC collateral parked in the manager's pooled balance. */
export const fetchManagerIdle = (managerId: string) =>
  get<{ manager_id: string; idle_raw: string; idle: number }>(`/api/predict/managers/${managerId}/idle-dusdc`);
/** Owner-signed reclaim of idle dUSDC back to the wallet (defaults to full idle). */
export const prepareManagerWithdraw = (b: { owner: string; manager_id: string; amount_raw?: string }) =>
  post<PreparedTx & { amount_raw: string }>('/api/predict/manager/withdraw/prepare', b);
...
export const prepareLpWithdraw = (b: { owner: string; plp_coin_id?: string; shares_raw?: string }) =>
  post<PreparedTx>("/api/predict/lp/withdraw/prepare", b);
...
// A PredictManager is created ONCE per wallet and then reused forever. The
// indexer that backs fetchManagers() can lag right after creation, and it
// filters a SHARED object by its owner field — either can transiently return
// [] for a wallet that already has a manager, which would make the app sign a
// REDUNDANT create_manager. We persist the resolved id per-owner so a known
// manager is never re-created (a second, independent driver of the repeated
// create_manager sign the diagnosis found).
let managerScopePromise: Promise<string> | null = null;
function managerCacheKey(owner: string): Promise<string> {
  managerScopePromise ??= get<{ package_id: string; predict_object_id: string }>("/api/predict/config")
    .then((config) => `${config.package_id}:${config.predict_object_id}`)
    .catch(() => "unknown-deployment");
  return managerScopePromise.then(
    (scope) => `pelagos:predict-manager:${scope}:${owner.toLowerCase()}`,
  );
}
async function cachedManager(owner: string): Promise<string | null> {
  try {
    const key = await managerCacheKey(owner);
    return typeof window !== "undefined" ? window.localStorage.getItem(key) : null;
  } catch {
    return null;
  }
}
async function cacheManager(owner: string, id: string): Promise<string> {
  try {
    const key = await managerCacheKey(owner);
...
export async function ensureManager(owner: string, sign: SignFn): Promise<string> {
  // 0) a manager we already resolved for THIS owner — never re-create.
  const cached = await cachedManager(owner);
  if (cached) return cached;
  const existing = await fetchManagers(owner).catch(() => []);
  if (existing.length > 0) return await cacheManager(owner, existing[0].manager_id);
  const prep = await prepareManager(owner);
  // NOTE: sign() may throw if the wallet is locked / its session expired
  // ("Incorrect password" from Slush). We let it propagate untouched — the
  // caller surfaces it via friendlyWalletError. We do NOT retry create here, so
  // one failed unlock never cascades into repeated create_manager signatures.
  const digest = await sign(prep.tx_bytes);
  const c = await confirmPredict(digest);
  // If create_manager REVERTED on-chain, surface the real reason instead of
  // spinning the indexer-lag retry below and reporting a misleading "not indexed".
  if (!c.ok && !c.created_manager_id) throw new Error(`Manager creation failed on-chain (${c.status}).`);
  if (c.created_manager_id) return await cacheManager(owner, c.created_manager_id);
  // fall back to a re-lookup (indexer lag)
  for (let i = 0; i < 5; i++) {
    await new Promise((r) => setTimeout(r, 1500));
    const m = await fetchManagers(owner).catch(() => []);
    if (m.length > 0) return await cacheManager(owner, m[0].manager_id);
  }
