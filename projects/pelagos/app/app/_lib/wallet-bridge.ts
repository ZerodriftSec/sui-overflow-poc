...
export function useUsdcBalance() {
  ...
  // Track the pending error-retry timer so the effect cleanup can cancel an
  // orphaned retry the instant the address changes — otherwise a retry queued
  // for WalletA could fire (and overwrite the balance) after switching to WalletB.
  ...
  const refresh = useCallback(async () => {
    ...
    const requestedAddress = address; // capture so a late response can detect a switch
    inFlight.current = true;
    setState((prev) => ({ ...prev, loading: true, error: null }));
    try {
      // Real mUSDC balance of the CONNECTED wallet (not the deployer).
      const res = await fetch(`${BACKEND_URL}/api/dev/balances/${requestedAddress}`);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = (await res.json()) as { usdc?: number };
      // Drop the result if the wallet changed while this fetch was in flight —
      // never let WalletA's balance land on WalletB's state.
      if (requestedAddress !== address) return;
      setState({ uiAmount: Number(data.usdc ?? 0), loading: false, error: null });
    } catch (err) {
      if (requestedAddress !== address) return; // wallet changed; don't retry the stale addr
      ...
      retryTimer.current = window.setTimeout(() => void refresh(), 2_500);
    } finally {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (retryTimer.current) window.clearTimeout(retryTimer.current); // cancel any orphaned retry on address change
...
export function useDusdcBalance() {
  ...
  const inFlight = useRef(false);
  // Same orphaned-retry guard as useUsdcBalance — clear the error-retry timer on
  // address change so a retry queued for the old wallet can't fire for the new one.
  const retryTimer = useRef<number | null>(null);
  ...
  const refresh = useCallback(async () => {
    ...
    setState((prev) => ({ ...prev, loading: true, error: null }));
    try {
      const res = await fetch(`${BACKEND_URL}/api/dev/dusdc-balance/${requestedAddress}`);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const data = (await res.json()) as { dusdc?: number };
      if (requestedAddress !== address) return; // wallet switched mid-flight; discard
 ...
 * dUSDC, and 3 SUI for gas. Returns the dispensed amounts + tx digest.
 */
