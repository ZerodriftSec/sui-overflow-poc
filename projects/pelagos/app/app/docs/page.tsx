  ...
  return (
    <>
      <P>
        ...
        diversified baskets, floor-targeted notes, and seniority slices —
  ...
  return (
    <>
      <P>
        ...
        Move packages on Sui testnet — including a Pelagos-operated deployment
        of Mysten&apos;s official DeepBook Predict source. The frontend never
        contacts upstream pricing or settlement directly; all on-chain reads,
        PTB builds, oracle health, and external data flow through the backend.
      </P>
  ...
  return (
    <>
      <P>
        ...
        <B>volatility-surface oracle</B>. Pelagos derives a conservative SVI
        surface from Deribit&apos;s public BTC market data and writes price,
        forward, and SVI updates to its authorized on-chain oracle. Every
        strike, probability, and greek shown by the app is then read from the
        same on-chain oracle state the contract uses for trading and settlement.
      </P>
      ...
      <P>
        Pelagos maintains a rolling ladder of four future BTC expiries selected
        from Deribit&apos;s listed markets. It reads the active set from{" "}
        <Code>GET /api/predict/oracles</Code> and snaps a requested strike to
        the nearest level on each oracle&apos;s on-chain grid. The{" "}
      ...
      <P>
        ...
        a winning range permissionlessly. At expiry, the managed feed submits
        the Deribit BTC spot to the authorized oracle; the contract freezes it
        as the settlement price and applies its own payout logic.
      </P>
    </>
  );
}
...
function PDistributed() {
  return (
    <>
      ...
      <P>
        Calls and puts are laid out across the managed on-chain expiry ladder,
        which rolls forward from Deribit&apos;s listed BTC markets. Each strike shows the live
...
function PNotes() {
  return (
    <>
      <P>
        <B>What it is.</B> A Protected Note pairs a <B>target floor reserve</B>{" "}
        with an <B>upside sleeve</B>. You commit a budget; most of it supplies
        the Predict PLP vault and the remainder buys an upside position that
        participates if your view plays out.
      </P>
      <SubHeading>When you&apos;d use it</SubHeading>
      <P>
        When you want a larger floor sleeve with some participation in the
        upside. You give up some of the upside from an outright position in
        exchange for allocating most of the budget to PLP.
      </P>
      <SubHeading>How the split works</SubHeading>
      <UL
        items={[
          <>
            <B>Floor portion.</B> The larger share of your deposit supplies
            Predict PLP, sized to your chosen floor target.
          </>,
          <>
      ...
      />
      <SubHeading>What the floor means</SubHeading>
      <P>
        Important and honest: the floor is a <B>target backed by a PLP reserve</B>,
        not an on-chain-enforced principal guarantee. Its realized value depends
        on PLP performance, while the separate strip can expire worthless. The
    ...
    <>
      ...
      <Table
        ...
        rows={[
          [
            "dUSDC",
            "DeepBook Predict&apos;s native testnet quote asset.",
            "From the testnet faucet — it is not freely mintable by Pelagos.",
          ],
          [
            "Pelagos USDC (mUSDC)",
            "Pelagos&apos;s testnet settlement asset for product testing.",
      ...
      <P>
        <B>mUSDC is Pelagos&apos;s testnet settlement asset.</B> It uses the{" "}
        <B>same live DeepBook pricing and payout schedule</B> as a dUSDC trade,
        while settlement runs through the lighter Pelagos Sui vault rail. mUSDC
        is freely mintable, so it avoids the native dUSDC faucet bottleneck while
        keeping the product economics directly comparable.
      </P>
...
function ChainPackages() {
  return (
    <>
      <P>
        Pelagos deploys its Move packages to Sui testnet together with an exact
        build of Mysten&apos;s official DeepBook Predict testnet source.
      </P>
      <Table
        ...
        rows={[
          ...
          [
            "DeepBook Predict (official Mysten source)",
            "predict, predict_manager, PLP vault, OracleSVI",
            "Pelagos-operated deployment for on-chain range pricing and settlement.",
          ],
        ]}
      />
...
function ChainDeploy() {
  return (
    <>
      ...
      />
      <SubHeading>Managed DeepBook Predict</SubHeading>
      <Table
        cols={["Object", "ID"]}
        rows={[
          ["Predict package", "0xf5716c88…2cb34"],
          ["Predict object (market root)", "0xc075248d…fa9794"],
          ["dUSDC type", "0xe9504008…73e1a::dusdc::DUSDC"],
          ["Reads", "Direct Sui gRPC + devInspect"],
          ["Oracle source", "Deribit → Pelagos feed → Sui"],
        ]}
      />
      <SubHeading>Verified end-to-end</SubHeading>
      <P>
        The vendored official Predict source passes 34 Move tests. Live
        testnet proofs include a binary mint and redeem plus a one-signature
        four-leg range-strip open and full redemption through the HTTP-bound
        execution path. Direct Sui object reads confirmed the exact opened
        quantities and zero residual quantity after redemption. Full package,
        object, source-commit, and transaction details are in{" "}
...
function RiskSummary() {
  return (
    <>
      ...
      <P>
        Settlement follows the managed DeepBook Predict oracle. Pelagos&apos;s
        authorized feed derives SVI from Deribit market data and submits the
        expiry spot used for settlement. A Deribit outage, feed-key compromise,
        bad model input, or delayed settlement propagates to every position
        referencing the affected market.
      </P>
      ...
      <P>
        ...
        been audited. The Predict deployment is built from Mysten&apos;s exact
        official testnet source, but Pelagos operates its upgrade and oracle
        capabilities.
      </P>
  ...
  return (
    <>
      ...
      <Faq
        ...
        a="DeepBook Predict is Mysten's on-chain prediction protocol — it exposes simple binary and range contracts over a live BTC volatility-surface oracle. Pelagos deploys the exact official testnet source, operates the authorized oracle feed, and adds a structured-product layer on top: options chains, volatility structures, distribution views, tranches, notes, and diversified baskets, each assembled from real on-chain contracts and minted in one signature where possible."
      />
      <Faq
      ...
      <Faq
        ...
        a="You pick a settlement rail per trade. dUSDC is DeepBook Predict's native testnet quote asset and mints the Predict position directly. Pelagos USDC (mUSDC) is freely mintable and uses the same live pricing and payout schedule with settlement through the Pelagos Sui vault. Both are treated 1:1 in USD and work end to end; use dUSDC for the native Predict path and mUSDC when you want the lighter testnet rail."
      />
      <Faq
      ...
      <Faq
        ...
        a="Pelagos maintains four future BTC expiries selected from Deribit's listed markets. The feed settles and compacts expired oracles, then creates and activates replacements so the ladder rolls forward automatically."
      />
      <Faq
      ...
      <Faq
        ...
        a="Each leg is a $1 binary: at settlement, in-the-money legs pay $1 each and out-of-the-money legs pay nothing, so the total payout is the sum across the position's legs. At expiry the managed feed submits Deribit BTC spot to the authorized oracle; the Predict contract freezes that value and applies its payout logic."
      />
      <Faq
        q="Where can I verify it actually works on chain?"
        a="DEPLOYMENT.md lists the live testnet package and object IDs, exact upstream source commit, and verified transaction digests for binary mint/redeem and a one-signature four-leg strip open/full redemption. Direct Sui object reads confirm the resulting quantities, and every prepared build dry-runs on chain before signing."
      />
    </>
  );
}
