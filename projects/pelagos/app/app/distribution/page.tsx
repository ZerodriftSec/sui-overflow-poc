...
function BasicOptionsChain() {
  ...
  useEffect(() => {
    ...
    if (currency !== "dUSDC" || !selOracle || !selExpiry || !selLower || !selHigher || !selQuote?.tradeable) { setDepth(null); return; }
    let alive = true;
    setDepth(null);
    fetchBandDepth({ oracle_id: selOracle, expiry: selExpiry, lower: selLower, higher: selHigher })
      .then((d) => { if (alive) setDepth(d); })
      .catch(() => { if (alive) setDepth(null); });
    return () => { alive = false; };
  }, [currency, selOracle, selExpiry, selLower, selHigher, selQuote?.tradeable]);
  ...
  // Balance gate (mirrors the volatility ticket): block opening when the SELECTED
  // currency's balance can't cover the order cost — switching currency mid-quote
  // could otherwise let an under-funded order through to a misleading wallet error.
  const bal = currency === "mUSDC" ? musdc.uiAmount : usdc.uiAmount;
  const shortBal = wallet.connected && nContracts >= 1 && bal + 1e-9 < orderCost;
  ...
  async function openStrike() {
    ...
    // Belt-and-suspenders balance gate: fail with a clear, upfront message instead
    // of the misleading "reconnect your wallet" path when under-funded.
    if (bal + 1e-9 < orderCost) {
      setOpenErr(`Insufficient ${currency} — you have ${bal.toFixed(2)} but this costs ${orderCost.toFixed(2)}.`);
      return;
    }
    ...
    try {
      ...
      // The wallet returns a digest even for a tx that REVERTED on-chain (e.g.
      // InsufficientGas). confirmPredict verifies effects.status; fail closed so a
      // reverted open never reports success or records a phantom position.
    ...
    <>
      ...
      <PageFrame wide>
        <div className="oc">
          ...
          {loadErr && !chain && (
            <div className="oc-err">DeepBook Predict pricing is temporarily unavailable. <span>{loadErr}</span></div>
          )}
            ...
            <div className="oc-card oc-ticket" style={selStrike && selQuote && selExp ? { alignSelf: "start" } : undefined}>
              {!selStrike || !selQuote || !selExp ? (
                <div className="oc-ticket-empty">
                  ...
                  <p className="oc-empty-note">Premiums are real range-option prices, quoted 0–1 per $1 contract, with dUSDC or Pelagos mUSDC settlement.</p>
                </div>
              ) : (
                <>
                  ...
                  <div className="oc-rows-info">
                    ...
                    {wallet.connected && <Info k="Balance" v={`${bal.toLocaleString(undefined, { maximumFractionDigits: 2 })} ${currency}`} color={shortBal ? C.amber : undefined} />}
                  ...
                  {openable ? (
                    <>
                      ...
                      {!wallet.connected ? (
                        <ConnectModal defaultOpen={false} trigger={<button className="oc-open">Connect a wallet to trade</button>} />
                      ) : (
                        <button className="oc-open" disabled={busy || nContracts < 1 || overCap || shortBal} aria-busy={busy} onClick={openStrike} style={{ opacity: busy || nContracts < 1 || overCap || shortBal ? 0.6 : 1, cursor: busy ? "wait" : "pointer" }}>
                          {busy ? (stage ?? "Submitting…") : shortBal ? `Need more ${currency} to open` : overCap ? `Exceeds pool depth · max ${maxContracts!.toLocaleString()}` : `Buy ${nContracts} ${sel!.side} ${nContracts === 1 ? "contract" : "contracts"} · $${orderCost.toFixed(2)}`}
                        </button>
                      )}
                      <p className="oc-note">{currency === "mUSDC"
                        ? `Buys ${nContracts} whole ${sel!.side} ${nContracts === 1 ? "contract" : "contracts"}, priced live off DeepBook and settled through the Pelagos mUSDC vault on Sui.`
...
function Info({ k, v, color }: { k: string; v: string; color?: string }) {
  return <div className="oc-info"><span>{k}</span><b style={color ? { color } : undefined}>{v}</b></div>;
}
...
const OC_CSS = `
  ...
  @media (max-width: 520px) {
    .oc-chain-head, .oc-cols, .oc-row { grid-template-columns: minmax(0, 1fr) 90px minmax(0, 1fr); }
    .oc-chain-head { padding-left: 6px; padding-right: 6px; }
    .oc-colhead, .oc-cells { grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 2px; padding-left: 5px; padding-right: 5px; min-width: 0; }
    .oc-colhead span { font-size: 7.5px; letter-spacing: 0.04em; }
    .oc-cells span { min-width: 0; font-size: 10px; }
    .oc-k { min-width: 0; font-size: 11px; }
  }
`;
...
function AdvancedDistribution() {
  ...
  const [poolCapUsd, setPoolCapUsd] = useState<number | null>(null); // contract-enforced remaining risk capacity
  ...
  useEffect(() => {
    ...
    fetch(`${BACKEND_URL}/api/predict/vault/summary`)
      ...
      .then((v) => {
        ...
        const remaining = Number(v?.remaining_risk_capacity);
        if (Number.isFinite(remaining) && remaining > 0) setPoolCapUsd(remaining / 1e6);
      })
      .catch(() => {});
  }, []);
  ...
  const predictUnavailable = /not currently quoteable|PREDICT_UNAVAILABLE/i.test(error ?? "");
  // Range buckets are mutually exclusive, so the contract's worst-case liability
  // is the largest realizable bucket payout, not the sum of every bucket.
  const stripMaxPayout = quote ? r6(quote.realized_max_payout_raw) : 0;
  ...
  const lock = quote ? r6(quote.total_cost_raw) : 0; // what you pay (escrowed at open)
  ...
  // Balance gate (mirrors the volatility ticket): block opening when the SELECTED
  // currency's balance can't cover `lock` — guards the currency-switched-mid-quote case.
  const bal = currency === "mUSDC" ? musdc.uiAmount : usdc.uiAmount;
  const shortBal = wallet.connected && !!quote && bal + 1e-9 < lock;
  const canOpen = wallet.connected && !!quote && !flat && !busy && !overPoolCap && !shortBal && !predictUnavailable;
  ...
  async function open() {
    ...
    // Belt-and-suspenders balance gate — clear upfront message over a wallet error.
    if (bal + 1e-9 < lock) { setError(`Insufficient ${currency} — you have ${bal.toFixed(2)} but this costs ${lock.toFixed(2)}.`); return; }
    setBusy(true);
    setError(null);
    setResult(null);
    try {
      ...
      <PageFrame wide zoom={0.8}>
        <div style={{ marginBottom: 22 }}>
          ...
          <p style={{ fontFamily: FS, fontSize: 14.5, color: C.textSecondary, margin: "8px 0 0", maxWidth: 680, lineHeight: 1.6 }}>
            ...
            f(x) is the latest indexed implied distribution from the DeepBook SVI surface. Move μ and σ to set your view
        ...
        <div className="dc-grid">
          ...
          <aside style={{ display: "flex", flexDirection: "column", gap: 16, minWidth: 0 }}>
            <div style={PANEL}>
              <div className="dc-cap">Markets · BTC tenors</div>
              <div className="dc-market-scroll">
                  ...
                  return (
                    <button key={m.id} onClick={() => selectMarket(m)} className="dc-market" style={{ borderColor: on ? C.tealLight : C.border, background: on ? C.cardHover : "transparent" }}>
                      ...
                      <div style={{ display: "flex", gap: 10, marginTop: 2, fontFamily: FM, fontSize: 10, color: C.textMuted }}>
                        ...
                        <span style={{ color: predictUnavailable ? C.amber : `${C.green}cc` }}>
                          {predictUnavailable ? "reference only" : "live"}
                        </span>
                      </div>
                    </button>
                  );
                ...
                {markets.length === 0 && (
                  <span style={{ fontFamily: FS, fontSize: 12.5, color: C.textMuted, lineHeight: 1.5 }}>
                    {loadErr ? "DeepBook Predict pricing is temporarily unavailable." : "Loading BTC tenors…"}
                  </span>
                )}
              </div>
            </div>
                ...
                <div style={{ display: "grid", gap: 18, marginTop: 14 }}>
                  ...
                  <div>
                    <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 8, marginBottom: 6 }}>
                      ...
                      {/* Pool-depth cap is a DeepBook Predict liability and binds the
                          dUSDC rail only. mUSDC settles on the Pelagos vault, so no
                          Predict-pool cap applies there. */}
                  ...
                  <button onClick={() => selectMarket(market)} className="dc-reset">Reset to indexed forward</button>
                </div>
                ...
                <div style={{ display: "grid", gap: 14, paddingTop: 18 }}>
                  ...
                  <div style={{ borderTop: `0.5px solid ${C.border}`, paddingTop: 12, display: "grid", gap: 6 }}>
                    ...
                    {wallet.connected && (
                      <div style={{ display: "flex", justifyContent: "space-between", fontFamily: FM, fontSize: 11, color: C.textMuted }}>
                        <span>Balance</span>
                        <span style={{ color: shortBal ? C.amber : C.textPrimary }}>{bal.toLocaleString(undefined, { maximumFractionDigits: 2 })} {currency}</span>
                      </div>
                    )}
                  </div>
                </div>
          ...
          <main style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 12 }}>
                ...
                {market && (
                  <div style={{ fontFamily: FM, fontSize: 11, color: C.textMuted }}>
                    {predictUnavailable ? "Indexed SVI reference · pricing unavailable" : `SVI surface · ${quoting ? "pricing…" : "121-pt grid"}`}
                  </div>
                )}
              </div>
            ...
            <div style={PANEL}>
              ...
              {!wallet.connected ? (
                <ConnectModal defaultOpen={false} trigger={<button className="dc-open" style={{ marginTop: 18, cursor: "pointer" }}>Connect a wallet to trade</button>} />
              ) : (
                <button onClick={open} disabled={!canOpen} aria-busy={busy} className="dc-open" style={{ marginTop: 18, opacity: canOpen ? 1 : 0.5, cursor: canOpen ? "pointer" : "not-allowed" }}>
                  {busy ? stage ?? "Submitting…" : predictUnavailable ? "DeepBook pricing unavailable" : flat ? "Widen σ to build a tradeable strip" : overPoolCap ? `Exceeds pool depth · reduce budget` : shortBal ? `Need more ${currency} to open` : `Open strip · lock ${quote ? usd(lock) : ""}`}
                </button>
              )}
