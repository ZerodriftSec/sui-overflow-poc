...
export default function VolatilityPage() {
  ...
  // Testnet default stays executable against the managed vault and fits inside
  // one 25 dUSDC test-funds allocation. Users can still size upward explicitly.
  ...
  const predictUnavailable = /not currently quoteable|PREDICT_UNAVAILABLE/i.test(err ?? "");
          ...
          <div className="vd-top">
            <div>
              ...
              <p>
                {mode === "advanced"
                  ? predictUnavailable
                    ? "The latest indexed SVI surface remains available for analysis; on-chain structure pricing is temporarily unavailable."
...
function BasicDesk(p: DeskProps) {
  ...
  const predictUnavailable = /not currently quoteable|PREDICT_UNAVAILABLE/i.test(p.err ?? "");
    ...
    <div className="vd-grid">
      ...
      <div className="vd-side">
        ...
        <div className="vd-card vd-ticket">
          ...
          {!wallet.connected ? (
            <ConnectModal defaultOpen={false} trigger={<button className="vd-open-btn" style={{ background: accent }}>Connect a wallet</button>} />
          ) : (
            <button className="vd-open-btn" style={{ background: accent }} disabled={busy || !q || tradeable === 0} aria-busy={busy} onClick={() => setShowExecute(true)}>
              {predictUnavailable ? "DeepBook pricing unavailable" : `Open ${meta.label} · ${q ? usd(q.strip.total_cost_raw) : ""}`}
            </button>
          )}
        </div>
      </div>
    ...
    <>
      {!wallet.connected ? (
        ...
        <button className="vd-open-btn" style={{ background: accent }} disabled={busy || !q || tradeable === 0} aria-busy={busy} onClick={openPosition}>
          {busy ? (stage ?? "Submitting…") : predictUnavailable ? "DeepBook pricing unavailable" : `Open ${advStructLabel} · ${q ? usd(q.strip.total_cost_raw) : ""}`}
        </button>
      )}
...
const VD_CSS = `
  ...
  @media (max-width: 600px) {
    .vd-analytics > .vd-card-head { flex-direction: column; align-items: stretch; }
    .vd-tabs { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); width: 100%; }
    .vd-tab { padding: 5px 4px; }
    .vd-analytics > .vd-card-head .vd-dim { line-height: 1.4; }
  }
