# Sui Overflow — Local PoC Verification Suite (Public)

This repository contains **local `sui move test` PoCs** for every human-confirmed
finding in the Sui Overflow audit group whose target contract is **NOT deployed
on Sui mainnet** (testnet-only or pre-mainnet projects).
...
The full audit covered 17 projects / 35 findings. 4 of those projects are
**live on Sui mainnet** today (talos, sweem, talise, memforks). Those PoCs and
the deployment package IDs are kept in a separate **private** repository
(`sui-overflow-poc-all`) to avoid publishing live-exploit chains against
production TVL.

This public repo holds the **non-mainnet** subset: 13 projects where the
vulnerable code is currently only on testnet or pre-deployment. Once a project
ships to mainnet, its PoCs graduate to the private repo.
...
`sui` CLI 1.76.0 (`/Users/chris/.local/bin/sui`).
...
A separate **private** repository (`sui-overflow-poc-all`) holds the full set
of 35 findings across all 17 audited projects, including the 4 projects that
are live on Sui mainnet today (talos, sweem, talise, memforks).
